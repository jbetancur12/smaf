...
const Templates: React.FC = () => {
  ...
  const [customerKey, setCustomerKey] = useState<string | null | undefined>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (customer !== null) {
      getCustomerTemplates(customer).then((res) => setTemplates(res));
    }
    getCustomer(customer).then((res) => setCustomerKey(res?.customerKey));
  }, []);
          ...
          {row.map((template: TemplateDataResponse) => (
            <Grid item xs={12} sm={6} md={4} lg={2} key={template._id}>
              <Link
                ...
                to={{
                  pathname: "charts",
                  search: `?customer=${customer}&template=${template._id}&customerKey=${customerKey}&templateKey=${template.templateKey}`,
                }}
              >
                <Card>
