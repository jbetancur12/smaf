...
import {
  doCreateControllerType,
  retrieveControllerTypes,
} from "@app/store/slices/controllerTypeSlice";
...
const ControllerTypes = () => {
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params) => {
        console.log(
          "🚀 ~ file: ControllerTypes.tsx:45 ~ ControllerTypes ~ params:",
          params
        );
        if (params.value) {
          ...
          return (
            <Link
              to={controllerTypeUrl}
              className="tw-no-underline tw-text-inherit tw-transition-transform tw-transform tw-hover:scale-110"
            >
              {params.row.name}
            </Link>
          );
    ...
    },
    {
      ...
      renderCell: () => (
        <>
          <IconButton className="tw-mr-2 " color="warning">
            <EditIcon />
          </IconButton>
    ...
    dispatch(doCreateControllerType(values))
      ...
      .then((res) => {
        if (res !== undefined) {
          ...
          error(
            "La respuesta es indefinida o no contiene la propiedad 'data'."
          );
        }
      })
      .catch((err) => {
  ...
  return (
    <div>
      ...
      >
        {controllerTypes && controllerTypes.length > 0 ? (
          <DataGrid
            rows={controllerTypes}
            columns={columns}
          ...
          <Typography variant="body1">
            Aun no hay controladores registrados{" "}
          </Typography>
        )}
      </Box>
      <Form
