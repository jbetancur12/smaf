...
const TemplatesPage = () => {
    ...
    dispatch(doDeleteVariable(variableToDelete._id))
      ...
      .then((_data) => {
        const updatedVariables = variables.filter(
          (item) => item._id !== variableToDelete._id
        );
  ...
  const handleFormSubmit = (values: any) => {
    dispatch(
      doCreateVariable({ ...values, customer: id, template: idTemplate })
    )
      .unwrap()
      .then((res) => {
        if (res !== undefined) {
  ...
  const handleFormEdit = (values: any) => {
    dispatch(doUpdateVariable({ id: values._id, data: values }))
      ...
      .then((data: VariableDataResponse) => {
        // setVariables(data)
        const updatedVariables: VariableDataResponse[] = variables.map(
          (item) => {
    ...
    <Box m="1.5rem 2.5rem">
      ...
      >
        ...
        >
          ...
          <p>
            ¿Está seguro de que desea eliminar la compañía{" "}
